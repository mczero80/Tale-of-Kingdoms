            ...
            if(arrayList.isEmpty() && arrayList1.isEmpty()) {
               ...
               for(int i = 0; i < x.speed; ++i) {
                  if(this.index < arrayList.size()) {
                     ...
                     if(i$ != null && world.getBlock(x.x + i$.posX, x.y + i$.posY, x.z + i$.posZ) != i$) {
                        if(i$.blockID == 0) {
                           world.setBlockToAir(x.x + i$.posX, x.y + i$.posY, x.z + i$.posZ);
                        }

                        if(i$ != Blocks.torch && i$ != Blocks.wooden_door && i$ != Blocks.ladder && i$ != Blocks.trapdoor) {
                           world.setBlock(x.x + i$.posX, x.y + i$.posY, x.z + i$.posZ, i$, i$.metadata, 3);
                        } else {
                     ...
                     Iterator var9 = this.torchList.iterator();

                     while(var9.hasNext()) {
                        FakeBlock var10 = (FakeBlock)var9.next();
                        world.setBlock(x.x + var10.posX, x.y + var10.posY, x.z + var10.posZ, var10, var10.metadata, 3);
                     }
